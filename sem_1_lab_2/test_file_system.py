...
def test_can_append_line_to_log_text_file():
    ...
    log_content_response = c.get('/logtextfile/read', json={"path": "/hello/file.log"})
...
def test_can_delete_directory():
    ...
    list_response = c.get('/directory/list', json={"path": "/work"})
    assert "projects" not in list_response.get_json()

    list_response_2 = c.get('/directory/list', json={"path": "/work/projects"})
...
def test_can_move_directory():
    ...
    assert "projects" in c.get("/directory/list", json={"path": "/work/new"}).data.decode("utf-8")
    assert c.get("/binaryfile/read", json={"path": "/work/new/projects/file.bin"}).data.decode("utf-8") == "some text"
...
def test_can_move_log_text_file(): 
    ...
    assert move_response.status_code == 200
    assert "file.log" in c.get('/directory/list', json={"path": "/hello/bye"}).data.decode("utf-8")
    assert c.get("/logtextfile/read", json={"path": "/hello/bye/file.log"}).data.decode("utf-8") == "some text"
...
def test_can_move_bin_file(): 
    ...
    assert move_response.status_code == 200
    assert "file.bin" in c.get('/directory/list', json={"path": "/hello/bye"}).data.decode("utf-8")
    assert c.get("/binaryfile/read", json={"path": "/hello/bye/file.bin"}).data.decode("utf-8") == "some text"
...
def test_can_move_buffer_file():
    ...
    assert move_response.status_code == 200
    assert "file.buf" in c.get('/directory/list', json={"path": "/hello/bye"}).data.decode("utf-8")
